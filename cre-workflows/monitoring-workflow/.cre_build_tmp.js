...
function postAlert(confClient, runtime2, severity, message, ratio) {
  ...
  if (!ok(resp)) {
    runtime2.log(`POST /alerts failed: status=${resp.statusCode}`);
  }
}
var onCronTrigger = (runtime2, _payload) => {
  ...
  runtime2.log(`Fetching monitoring: ${monitoringUrl}`);
  const confClient = new cre.capabilities.ConfidentialHTTPClient;
  const monResp = confClient.sendRequest(runtime2, {
    vaultDonSecrets: [],
    request: {
      url: monitoringUrl,
      method: "GET",
  ...
  if (!ok(monResp)) {
    runtime2.log(`Monitoring API failed: ${monResp.statusCode}`);
    return { status: "error", reserveRatio: 0, alertSent: false, safeguardTriggered: false };
  }
  const snapshot = json(monResp);
  runtime2.log(`Reserve ratio: ${snapshot.reserveRatio}, status: ${snapshot.status}`);
  let alertSent = false;
  let safeguardTriggered = false;
  if (snapshot.status === "warning") {
    postAlert(confClient, runtime2, "warning", "Reserve ratio below warning threshold (CRE workflow)", snapshot.reserveRatio);
    alertSent = true;
    runtime2.log("Warning alert sent to backend");
  }
  if (snapshot.status === "critical") {
    postAlert(confClient, runtime2, "critical", "Reserve ratio below critical threshold (CRE workflow)", snapshot.reserveRatio);
    alertSent = true;
    runtime2.log("Critical alert sent to backend");
    if (config.evm) {
      runtime2.log(`Triggering on-chain safeguard on ${config.evm.chainSelectorName}`);
      ...
      if (!net) {
        runtime2.log(`Network not found: ${config.evm.chainSelectorName}`);
      ...
      const contractAddr = config.evm.reserveMonitorAddress;
      ...
      try {
        ...
        const readResp = evmClient.callContract(runtime2, {
        ...
        runtime2.log("Could not read withdrawalsPaused state, proceeding with pause");
      }
      if (isPaused) {
        runtime2.log("Withdrawals already paused, skipping on-chain call");
      } else {
        evmClient.callContract(runtime2, {
          call: encodeCallMsg({ from: zeroAddress, to: contractAddr, data: callData }),
          blockNumber: LAST_FINALIZED_BLOCK_NUMBER
        }).result();
        runtime2.log("On-chain pauseWithdrawals triggered");
      }
